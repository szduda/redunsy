...
const compileBar = (bar: string, groove: string) => {
  const grooveLength = groove.length
  const notationCells = barCellCount(bar)
  const cells = parseBar(bar, grooveLength)
...
export const compileGroove = ({ bars, groove }: CompileGrooveInput): CompileGrooveResult => {
  if (!groove.length) throw new Error('Groove pattern cannot be empty')

  const cellsPerBar = groove.length
  const compiledBars = bars.map((bar) => compileBar(bar, groove))
