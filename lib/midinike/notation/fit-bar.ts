...
export const validateBarsForGroove = (bars: string[], grooveLength: number) => {
  barsCellCounts(bars).forEach((count, index) => {
    if (count > grooveLength) {
      throw new Error(
        `Bar "${bars[index]}" has ${count} cells, more than groove length ${grooveLength}`,
      )
    }
  })
}
...
export const barsMatchGrooveLength = (bars: string[], grooveLength: number) =>
  barsCellCounts(bars).every((count) => count === grooveLength)
